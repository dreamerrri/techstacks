            ...
            <div className="p-2 sm:p-4">
                <div className="flex justify-between items-center flex-wrap gap-3 mb-6">
                    <div>
                        ...
                        <p className="text-dim-foreground m-0">Track and review all system activity and changes.</p>
                    </div>
                </div>
                ...
                <div className="rounded-xl border border-edge bg-card p-4 mb-4">
                    <div className="flex flex-wrap items-end gap-3">
                        ...
                        <div className="flex gap-2">
                            <button onClick={applyFilters} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-danger/40 px-3 text-xs font-medium text-danger no-underline transition-colors hover:bg-danger/10">
                    ...
                    {logs.total > 0 ? (
                        <>
                                    ...
                                    <tbody>
                                        {logs.data.map((log) => (
                                            <tr key={log.id} className="row-hover">
                                                <td className="text-dim-foreground text-xs whitespace-nowrap">
                                                ...
                                                <td className="text-dim-foreground">{log.module ? log.module.charAt(0).toUpperCase() + log.module.slice(1) : ''}</td>
                                                <td className="text-dim-foreground max-w-[200px] truncate">{log.description}</td>
                                                <td className="text-dim-foreground font-mono text-xs">{log.ip_address || '—'}</td>
                                                <td className="text-center">
                                                    <Link href={`/audit-logs/${log.id}`} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-edge bg-dim px-3 text-xs font-medium no-underline transition-colors hover:bg-dim/60" title="View details">
                            ...
                            <div className="md:hidden p-4 flex flex-col gap-3">
                                {logs.data.map((log) => (
                                    <div key={log.id} className="rounded-xl border border-edge bg-card p-4">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-dim-foreground/70">{fmtDateTime(log.created_at)} {fmtTime(log.created_at)}</div>
                                                </div>
                                            </div>
                                            <span className={`badge ${actionBadge(log.action)} whitespace-nowrap`}>{log.action ? log.action.charAt(0).toUpperCase() + log.action.slice(1) : ''}</span>
                                        </div>
                                        ...
                                        <div className="text-xs text-dim-foreground/70 mt-1">{log.description}</div>

                                        <div className="mt-3 pt-3 border-t border-edge/60">
                                            <Link href={`/audit-logs/${log.id}`} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-highlight px-3 text-xs font-medium text-highlight-foreground no-underline transition-colors hover:bg-highlight/90">
                        ...
                        <div className="py-16 px-6 text-center">
                            <Icon name="tabler--history" className="size-10 text-dim-foreground/70 mx-auto mb-4" />
                            <h3 className="text-dim-foreground font-semibold mb-2">No Audit Logs Found</h3>
                            <p className="text-dim-foreground/70 mb-0">No audit log entries match your filters.</p>
                        </div>
                    )}
                </div>
            </div>
