...
@section('breadcrumb')
    ...
    <i class="fas fa-chevron-right text-xs"></i>
    <a href="{{ route('roles.show', $role) }}" class="text-white/55 no-underline hover:text-white">{{ $role->name }}</a>
    <i class="fas fa-chevron-right text-xs"></i>
    <span class="text-white font-semibold">Edit {{ $role->name }}</span>
@endsection
...
@section('content')
    ...
    <div class="card bg-base-100 shadow-sm p-6">
        <h2 class="text-base font-bold text-gray-800 mb-6 flex items-center gap-2">
            <i class="fas fa-edit text-red-600"></i> Edit — {{ $role->name }}
        </h2>
        ...
        <form method="POST" action="{{ route('roles.update', $role) }}">
            ...
            <div class="mb-8">
                ...
                @else
                    <p class="text-gray-400 text-sm m-0">No permissions available.</p>
                @endif
